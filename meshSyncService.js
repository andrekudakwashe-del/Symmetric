...
function calculateRecordHash(uuid, deviceId, timestamp, total) {
...
class MeshSyncManager {
  constructor() {
    ...
    this.peerListeners = [];
    this.virtualDevices = [];
    this.seenPacketIds = /* @__PURE__ */ new Set();
    this.opportunisticTimer = null;
    // Real Mesh Transport: MQTT Over WebSocket
    this.mqttClient = null;
    this.currentBrokerIndex = 0;
    this.isMqttConnected = false;
    this.brokerName = "Connecting...";
  ...
  saveConfig(branchCode, deviceName) {
  ...
  ingestOfflineBatchPayload(rawString) {
    try {
  ...
  async toggleTorch(targetState) {
    const newState = targetState !== void 0 ? targetState : !this.isTorchOn;
    try {
  ...
  async acceptWebRtcOffer(offerB64) {
    try {
      if (this.webrtcPeerConnection) {
        try {
          this.webrtcPeerConnection.close();
      ...
      const sdpDesc = parsed.sdp ? { type: "offer", sdp: parsed.sdp } : parsed;
      const pc = new RTCPeerConnection({ iceServers: [] });
      this.webrtcPeerConnection = pc;
  ...
  async completeWebRtcAnswer(answerB64) {
    try {
      if (!this.webrtcPeerConnection) return false;
      const raw = atob(answerB64);
      const parsed = JSON.parse(raw);
      const sdpDesc = parsed.sdp ? { type: "answer", sdp: parsed.sdp } : parsed;
  ...
  // =========================================================================
  handleIncomingMeshPacket(topic, packet) {
  ...
  async sendTestPing(customMessage) {
    const packetId = `PING-${Date.now()}`;
    this.seenPacketIds.add(packetId);
    const message = customMessage || `Live Ping from ${this.config.deviceName} at ${(/* @__PURE__ */ new Date()).toLocaleTimeString()}`;
    const packet = {
      type: "ping",
      packet_id: packetId,
      branchCode: this.config.branchCode,
  ...
  getOnlineDeviceCount() {
    ...
    for (const peer of this.discoveredPeers.values()) {
      if (now - peer.lastSeen <= 9e3) count++;
    }
    return count;
  }
  getRealPeers() {
  ...
  addLog(type, message, sourceDevice, targetDevice, ttl, uuid) {
    const log = {
  ...
  makeSaleOnDevice(deviceId, amount = 45, customerName = "Walk-in") {
    ...
    const queueItem = {
      ...
      syncStatus: "pending",
      hash,
      payload: { id: invoiceNumber, timestamp: (/* @__PURE__ */ new Date()).toISOString(), customerName, total: amount }
    };
    dev.pendingQueue.unshift(queueItem);
    this.addLog("BROADCAST", `Sale ${invoiceNumber} ($${amount.toFixed(2)}) created on ${dev.name}`, dev.name, void 0, 5, uuid);
  ...
  async triggerOpportunisticCloudSync(deviceId) {
    const targetDevs = deviceId ? this.virtualDevices.filter((d) => d.id === deviceId) : this.virtualDevices.filter((d) => d.isOnline && d.hasInternet);
  ...
  startOpportunisticCloudSyncTimer() {
  ...
  async executeOfficialTestPlan(onStepUpdate) {
    ...
    this.makeSaleOnDevice("DEV-VIRTUAL-1", 50, "Customer Alpha");
    this.makeSaleOnDevice("DEV-VIRTUAL-1", 120, "Customer Beta");
    this.makeSaleOnDevice("DEV-VIRTUAL-1", 35, "Customer Gamma");
    await new Promise((r) => setTimeout(r, 600));
    ...
    if (dev4) {
      dev4.isOnline = true;
      const dev12 = this.virtualDevices.find((d) => d.id === "DEV-VIRTUAL-1");
      if (dev12) {
        dev4.sales = [...dev12.sales];
        dev4.pendingQueue = [...dev12.pendingQueue];
      }
      this.notifyPeerSubscribers();
    }
    ...
    this.makeSaleOnDevice("DEV-VIRTUAL-3", 75, "Customer Delta");
    this.makeSaleOnDevice("DEV-VIRTUAL-4", 90, "Customer Epsilon");
    await new Promise((r) => setTimeout(r, 800));
...
const meshSyncService = new MeshSyncManager();
export {
  calculateRecordHash,
  meshSyncService
};
